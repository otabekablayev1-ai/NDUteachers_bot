...
async def fetch_answers_range(
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            ...
            .where(Question.created_at.between(date_from, date_to))
...
# =============================
# 🕓 So‘nggi yuborilgan savollar
# =============================
async def get_latest_questions(limit: int = 10) -> list[dict]:
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Question)
            .order_by(Question.created_at.desc())
...
async def save_commands_file(file_id: str) -> None:
    async with AsyncSessionLocal() as session:
        ...
        session.add(CommandsFile(file_id=file_id))
        await session.commit()
...
# =============================
# ♻ O‘QITUVCHI / TUTOR / STUDENT FILTER
# =============================
